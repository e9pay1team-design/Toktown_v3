...
- 편집: 빈 곳 드래그 = **카메라 패닝** · 오브젝트 탭 = 선택 · 드래그 = 이동(유효/무효 초록·빨강 커서) · 오브젝트 위 캔버스 버튼 **✓ 배치 확정 / ↻ 방향 전환(건물·랜드마크) / ✕ 보관함 반환**
- **↻ 방향 전환**: 매장 건물·랜드마크는 좌측 하단(sw, 기본) ↔ 우측 하단(se) 두 방향 — 건물은 문·어닝·간판이 해당 면으로 이동, 랜드마크는 좌우 미러. 문 앞 보행 칸 판정도 방향을 따라가며, 방향은 배치에 저장
- **문 앞 판정 예외**: 문 앞 칸에 **걸을 수 있는 바닥 타일**이 깔려 있는 것은 막힌 것으로 치지 않는다 (벤치 등 일반 소품은 여전히 차단)
- **바닥 타일 연속 배치**: 광장 돌바닥·기본 초록 타일은 ✓ 확정 시 보관함에 같은 타일이 남아 있으면 **직전 배치 → 이번 배치 방향(진행 방향)으로 다음 타일이 자동 준비**된다 — 한 방향으로 쭉 깔거나, 다른 칸으로 끌어 꺾으면 그 방향으로 이어진다
- **🎒 전체 회수**: 좌측 상단 버튼 — 확인 팝업 후 배치된 모든 오브젝트를 일괄 보관함 반환 (획득물은 유지, 마을 리셋 없이 레이아웃만 초기화)
- **배치 규칙**

| 오브젝트 | 풋프린트 | 충돌 | 제약 |
|---|---|---|---|
| 매장 건물 | 2×2 | 있음 | 잔디 위만, **문 앞 1칸 보행 가능해야 함** (방향별 판정), ↻ 방향 전환 |
| 랜드마크 미니어처 | 2×2 | 있음 | 잔디 위만 |
| 소품 (상점 구매) | 1×1 | **없음** (끼임 방지) | 잔디 위만, 다른 배치물과 겹침 불가 |
| 바닥 타일 (광장 돌바닥·기본 초록) | 1×1 | 없음 | **잔디 + 중앙 광장 돌바닥 위** — 광장을 초록 타일로 덮어쓰기 가능, 연속 배치 지원 |
