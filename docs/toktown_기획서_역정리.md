...
- 편집: 빈 곳 드래그 = **카메라 패닝** · 오브젝트 탭 = 선택 · 드래그 = 이동(유효/무효 초록·빨강 커서) · 오브젝트 위 캔버스 버튼 **✓ 배치 확정 / ↻ 방향 전환(매장 건물) / ✕ 보관함 반환**
- **↻ 건물 방향 전환**: 매장 건물은 좌측 하단(sw, 기본) ↔ 우측 하단(se) 두 방향 — 문·어닝·간판이 해당 면으로 이동하고, 문 앞 보행 칸 판정도 방향을 따라간다. 방향은 배치에 저장
- **바닥 타일 연속 배치**: 광장 돌바닥·기본 초록 타일은 ✓ 확정 시 보관함에 같은 타일이 남아 있으면 **방금 자리 옆에 다음 타일이 자동으로 준비**된다 (연속으로 쭉 깔기)
